...
import { fontSize, smallIconSize } from '../utils/styleVariables';
...
class ServingModifier extends React.Component<Props, State> {
  constructor(props: Props) {
    ...
    const maxServing =
      props.maxServing && props.maxServing > 0 ? props.maxServing : MAX_SERVING;
    const quantity =
      props.quantity && props.quantity < maxServing ? props.quantity : 1;
    this.state = {
      maxServing,
      quantity: 1,
  ...
  componentDidMount() {
    const quantity =
      this.props.quantity && this.props.quantity < this.state.maxServing
        ? this.props.quantity
        : 1;
    this.setState({
  ...
  onQuantityDecreased() {
    const newQuantity = this.state.isMinusDisabled
      ? 1
  ...
  onQuantityIncreased() {
    const newQuantity = this.state.isPlusDisabled
      ? this.state.maxServing
      : this.state.quantity + 1;
    this.setState({
      isMinusDisabled: this.isMinusDisabled(newQuantity),
      isPlusDisabled: this.isPlusDisabled(newQuantity, this.state.maxServing),
  ...
  render() {
    return (
      <div className="ServingModifier">
        <div
          className={`ServingModifierHandler--minus${
            this.state.isMinusDisabled ? '--disabled' : ''
          }`}
          onClick={this.onQuantityDecreased}
        />
        <div className="ServingModifierDisplay">{this.state.quantity}</div>
        <div
          className={`ServingModifierHandler--plus${
            this.state.isPlusDisabled ? '--disabled' : ''
          }`}
